...
import { useLocalTeams } from '../hooks/useLocalData';
...
const TeamsPage: React.FC<TeamsPageProps> = ({ onNavigate }) => {
  ...
  const { teams: allTeams, loading } = useLocalTeams();

  // Local search state
  const [searchText, setSearchText] = useState('');

  // Filter teams based on search
  const teams = searchText
        ...
        <div className="teams-header">
          <div className="teams-title-section">
            ...
            <p className="teams-subtitle">
              {allTeams.length > 0 ? `${allTeams.length} team${allTeams.length !== 1 ? 's' : ''}` : 'Manage your teams and players'}
            </p>
          </div>
